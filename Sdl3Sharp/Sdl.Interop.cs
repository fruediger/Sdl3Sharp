...
using unsafe AppEvent_func = delegate* unmanaged[Cdecl]<void*, Sdl3Sharp.Events.Event*, Sdl3Sharp.AppResult>;
using unsafe AppInit_func = delegate* unmanaged[Cdecl]<void**, int, byte**, Sdl3Sharp.AppResult>;
using unsafe AppInterate_func = delegate* unmanaged[Cdecl]<void*, Sdl3Sharp.AppResult>;
using unsafe AppQuit_func = delegate* unmanaged[Cdecl]<void*, Sdl3Sharp.AppResult, void>;
using unsafe Main_func = delegate* unmanaged[Cdecl]<int, byte**, int>;
...
partial class Sdl
{
	...
	internal unsafe static partial int SDL_EnterAppMainCallbacks(int argc, byte** argv, AppInit_func appinit, AppInterate_func appiter, AppEvent_func appevent, AppQuit_func appquit);
	...
	[NativeImportFunction<Library>(CallConvs = [typeof(CallConvCdecl)])]
	internal unsafe static partial int SDL_RunApp(int argc, byte** argv, Main_func mainFunction, void* reserved);
	...
	private unsafe int Run(AppBase app, int argc, byte** argv)
	{
		...
		unsafe static AppResult appevent(void* appstate, Event* @event)
		{
			if (appstate is not null && GCHandle.FromIntPtr(unchecked((IntPtr)appstate)) is { IsAllocated: true, Target: Sdl { mRunningApp: AppBase app } sdl } gcHandle)
			{
				try
				{
					return app.OnEventInternal(sdl, ref *@event);
				}
				catch
				{
					gcHandle.Free();
		...
		int main(int argc, byte** argv)
		{
			...
			try
			{
				return SDL_EnterAppMainCallbacks(argc, argv, unchecked((AppInit_func)Marshal.GetFunctionPointerForDelegate(appInitDelegate)), &appiterate, &appevent, &appquit);
			}
			finally
			{
				// just to make sure
		...
		try
		{
			...
			return SDL_RunApp(argc, argv, unchecked((Main_func)Marshal.GetFunctionPointerForDelegate(mainDelegate)), reserved: null);
		}
		finally
		{
			mRunningApp = null;
