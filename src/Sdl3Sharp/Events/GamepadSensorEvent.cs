...
public struct GamepadSensorEvent : ICommonEvent<GamepadSensorEvent>, IFormattable, ISpanFormattable
{
	...
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)] readonly get { unsafe { return MemoryMarshal.CreateReadOnlySpan(ref Unsafe.As<DataArray, float>(ref Unsafe.AsRef(in mData)), 3); } }
		...
		set
		{
			...
			value[..3].CopyTo(MemoryMarshal.CreateSpan(ref Unsafe.As<DataArray, float>(ref mData), 3));

			[DoesNotReturn]
			static void failValueArgumentTooSmall() => throw new ArgumentException($"The {nameof(Span<>.Length)} of the given {nameof(value)} argument is less than 3", paramName: nameof(value));
		}
	}
	...
	[InlineArray(3)] private struct DataArray { private float _; }
}
