...
/// </summary>
public sealed partial class TitleStorage : Storage
{	
	...
	/// <exception cref="SdlException">The <see cref="TitleStorage"/> could not be opened (check <see cref="Error.TryGet(out string?)"/> for more information)</exception>
	private unsafe TitleStorage(string? rootOverride, Properties? properties, IUnsafeConstructorDispatch? _ = default) :
		base(SDL_OpenTitleStorage(Utf8Convert(rootOverride, out var rootOverrideUtf8), properties?.Id ?? 0))
	{
		try
		{
			if (Pointer is null)
			{
				failCouldNotCreateTitleStorage();
			}
		}
		finally
		{
			Utf8StringMarshaller.Free(rootOverrideUtf8);
		}

		[DoesNotReturn]
		static void failCouldNotCreateTitleStorage() => throw new SdlException($"Could not open the {nameof(TitleStorage)}");
	}

	/// <summary>
	/// Creates a new <see cref="TitleStorage"/>
	/// </summary>
	/// <param name="rootOverride">A path to override the backend's default title root, or <c><see langword="null"/></c> to not override</param>
	/// <param name="properties">An optional group of properties that may contain backend-specific information</param>
	/// <remarks>
	/// <para>
	/// By default, <see cref="TitleStorage"/> uses the generic storage implementation.
	/// When the <paramref name="rootOverride"/> is not provided, the generic implementation will use the output of <see cref="FileSystem.BasePath"/> as the base path.
	/// </para>
	/// </remarks>
	/// <inheritdoc cref="TitleStorage(string?, Properties?, IUnsafeConstructorDispatch?)"/>
...
#pragma warning disable IDE0034 // Keep it that way for explicitness sake
		this(rootOverride, properties, default(IUnsafeConstructorDispatch?))
