...
/// </remarks>
#endif
public partial interface IRenderingDriver
{
	// We don't need to use CRTP or something like that here, just to prevent IRenderingDriver from being used as a generic type argument
	// IRenderingDriver contains a "static abstract" member, which is already enough to prevent it from being used as such,
	// since static abstract members are required to be implemented by type arguments, so only type that implement IRenderingDriver can be used as type arguments.
	// The only real issue with that is that a user would get a weird or confusing compile-time error message, if they try to use IRenderingDriver directly as a type argument,
	// but on the other hand, IRenderingDriver contains an "internal" member to be implemented, and if an user would try to implement IRenderingDriver themselves,
	...
	/// alternatively you can check the <see cref="RenderingDriverExtensions.get_IsAvailable{TDriver}"/> property for that.
	/// </para>
	/// </remarks>
		...
		get
		{
			...
			static ImmutableArray<string> buildAvailableDrivers()
			{
				unsafe
				{
					...
					{
						builder.Add(Utf8StringMarshaller.ConvertToManaged(SDL_GetRenderDriver(i))!);
					}
